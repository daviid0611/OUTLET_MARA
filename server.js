...
const productoSchema = new mongoose.Schema({
    ...
    precio: Number,
    categoria: String,
    etiquetaEstado: String, // Etiqueta estetica
    stock: Number,
    imagen: String
});
...
app.post('/api/nueva-venta', async (req, res) => {
    ...
    try {
        for (let item of carrito) {
            ...
            if (!prod) return res.status(400).json({ mensaje: `No encontrado: ${item.nombre}` });
        ...
        const nuevaVenta = new Venta({
            ...
            tipo, metodoPago: metodoPago || 'Efectivo', referenciaPago: referenciaPago || '',
            items: carrito, total: totalTransaccion,
            pagado: tipo === 'donacion' ? 0 : Number(pagoRealizado),
            deuda: Number(deuda), estado
        });
...
app.put('/api/ventas/:id', async (req, res) => {
    ...
    try {
        ...
        const nuevoTotal = Number(total); const nuevoPagado = Number(pagado); const nuevaDeuda = nuevoTotal - nuevoPagado;
        
        ventaAnterior.cliente = cliente; ventaAnterior.items = items; ventaAnterior.total = nuevoTotal; ventaAnterior.pagado = nuevoPagado; ventaAnterior.deuda = nuevaDeuda > 0 ? nuevaDeuda : 0; ventaAnterior.referenciaPago = referenciaPago || ventaAnterior.referenciaPago;
